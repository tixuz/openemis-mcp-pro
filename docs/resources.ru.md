---
title: OpenEMIS MCP Pro — Справочник ресурсов для управления образованием
description: Полный справочник 678 ресурсов OpenEMIS с доступностью HTTP-методов и статусом записи для MCP-сервера чтения и записи (Core 5.13.0).
keywords:
  - OpenEMIS
...
# OpenEMIS MCP Pro — Справочник ресурсов для управления образованием

> **678 ресурсов** · **3361 эндпоинтов** · v1.2.0 · OpenEMIS Core 5.13.0
>
> PRO-дистрибутив — чтение + запись. Ресурсы, управляемые workflow, заблокированы на уровне инструмента (🔒); используйте соответствующий playbook вместо прямой записи.
>
> MCP **не** загружает эту таблицу в контекст ИИ. Он использует компактный
> `grouped-manifest.json` (~210 KB) с доменными корзинами и playbooks.
> Агент перемещается по доменам и playbooks, подтягивая детали эндпоинтов по запросу.
...
|---|---|
| ✅ | Доступно |
| 🔒 | Только через workflow — используйте соответствующий playbook, не прямую запись |
| — | Недоступно для этого ресурса |
...
| `institution-student-survey-answers` | Institution* | ✅ | — | — | — | — (только чтение) |
...
| **── Task* ──** | | | | | | |
| `task-failures` | Task* | ✅ | — | — | — | — (только чтение) |
| `task-jobs` | Task* | ✅ | — | — | — | — (только чтение) |
| **── одиночные ──** | | | | | | |
